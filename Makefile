...
check:
	...
	@python -c "import bs4" >/dev/null 2>&1 && echo -e "$(GREEN)[✓] beautifulsoup4$(RESET)" || { echo -e "$(RED)[✗] beautifulsoup4$(RESET)"; python -m pip install -q beautifulsoup4; }
	...
	@python -c "import fake_useragent" >/dev/null 2>&1 && echo -e "$(GREEN)[✓] fake-useragent$(RESET)" || { echo -e "$(RED)[✗] fake-useragent$(RESET)"; python -m pip install -q fake-useragent; }
